...
// Rider completion events dedupe by home_services id; one Done per
// request. Used to silence the pasundo loop in the case where the rider
// skipped "On my way" and went straight to Done.
const seenPickupCompletes = new Set();
...
// Pahatid terminal-state events dedupe by transport_requests id; one
// silencer call per row even if downstream updates keep firing
// (status echoes, audit field stamps, etc.).
const seenTransportTerminals = new Set();
...
export function startPosTriggers() {
  return triggerSubscribe(async (change) => {
    ...
    // Transport request deletion — silence any pahatid loop hanging off
    // this row. Same protection as the homeServices delete above so a
    // hard-deleted request doesn't leave the chime ringing forever on
    // every rider's phone.
    if (change.entityType === 'transportRequests' && change.operation === 'delete' && change.entityId) {
      try {
        await NotificationService.stopLoopsForTransportRequest(change.entityId);
      } catch (e) { /* swallow */ }
    }
    ...
    if (!isRemote && change.entityType === 'homeServices' && change.operation === 'update' && change.entityId) {
      try {
        ...
        // Rider completion — pings the therapist that the rider has
        // arrived AND stops any pasundo loop still ringing on the
        // rider's device. Covers the case where the rider tapped Done
        // without first tapping "On my way", so the ack handler above
        // never fired and the loop kept playing. stopLoopsForHomeService
        // is idempotent — safe to call here even if the ack handler
        // already silenced it.
        if (hs && hs.pickupCompletedAt && !seenPickupCompletes.has(hs._id || hs.id)) {
          seenPickupCompletes.add(hs._id || hs.id);
          try { await NotificationService.stopLoopsForHomeService(hs._id || hs.id); } catch {}
          const rider = hs.pickupCompletedBy || 'Rider';
          const targetTherapistId = hs.pickupRequestedByUserId || null;
          if (targetTherapistId) {
            await NotificationService.notify({
              type: NotificationService.TYPES.POS_PICKUP_REQUEST,
              targetUserId: targetTherapistId,
              title: 'Rider arrived',
              message: `${rider} has arrived${hs.address ? ` at ${hs.address}` : ''}`,
              action: '/rooms',
              actionLabel: 'Open',
              soundClass: 'oneshot',
              payload: { homeServiceId: hs._id || hs.id },
    ...
    // Pahatid terminal-state silencer. The pahatid loop chimes on every
    // rider's phone until somebody answers; stop it the moment the row
    // lands on any terminal status:
    //   - completed  → rider tapped Done
    //   - cancelled  → requester voided
    //   - acknowledged → rider tapped "On my way" (the other riders no
    //                    longer need to compete for the same drop-off)
    // No isRemote gate here: the stopLoopsFor* helpers act on this
    // device's local Dexie state, so every device that received the
    // original loop must silence its OWN copy when the terminal state
    // arrives via realtime. Dedupe per row so subsequent updates
    // (e.g. cancelledBy stamp) don't try to re-dismiss already-dismissed
    // notifications.
    if (change.entityType === 'transportRequests' && change.operation === 'update' && change.entityId) {
      try {
        const all = await mockApi.transportRequests.getTransportRequests();
        const tr = (all || []).find((r) => (r._id || r.id) === change.entityId);
        if (tr && ['acknowledged', 'completed', 'cancelled'].includes(tr.status)
                && !seenTransportTerminals.has(tr._id || tr.id)) {
          seenTransportTerminals.add(tr._id || tr.id);
          try { await NotificationService.stopLoopsForTransportRequest(tr._id || tr.id); } catch {}
        }
      } catch (e) { /* swallow */ }
    }
