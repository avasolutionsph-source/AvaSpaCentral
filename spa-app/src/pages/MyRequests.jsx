...
const MyRequests = ({ embedded = false, onDataChange }) => {
  ...
  const loadRequests = async () => {
    ...
    try {
      ...
      setRequests({
        ot: otReqs.sort((a, b) => new Date(b.createdAt || b._createdAt || 0) - new Date(a.createdAt || a._createdAt || 0)),
        leave: leaveReqs.sort((a, b) => new Date(b.createdAt || b._createdAt || 0) - new Date(a.createdAt || a._createdAt || 0)),
        cashAdvance: cashReqs.sort((a, b) => new Date(b.createdAt || b._createdAt || 0) - new Date(a.createdAt || a._createdAt || 0)),
        incident: incidentReqs.sort((a, b) => new Date(b.createdAt || b._createdAt || 0) - new Date(a.createdAt || a._createdAt || 0))
      });
        ...
        <table className="requests-table">
          ...
          <tbody>
            {currentRequests.map(request => (
              <tr key={request._id}>
                <td>{safeFormatDate(request.createdAt || request._createdAt)}</td>

                {activeRequestType === 'ot' && (
                  <td>
                    <div>{safeFormatDate(request.date)}</div>
                    <div className="text-muted">{request.startTime || 'N/A'} - {request.endTime || 'N/A'}</div>
                  </td>
                )}

                {activeRequestType === 'leave' && (
                  <td>
                    <div style={{ textTransform: 'capitalize' }}>{request.type || 'N/A'}</div>
                ...
                {activeRequestType === 'cashAdvance' && (
                  <td className="number">
                    PHP {(Number(request.amount) || 0).toLocaleString('en-PH', { minimumFractionDigits: 2 })}
                  </td>
                )}

                {activeRequestType === 'incident' && (
                  <td>
                    <div>{request.title || 'N/A'}</div>
                    <div className="text-muted">{safeFormatDate(request.incidentDate)}</div>
                  </td>
                )}
