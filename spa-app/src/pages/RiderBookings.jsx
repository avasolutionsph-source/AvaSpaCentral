...
export default function RiderBookings() {
  ...
  // Per-rider hide list. Riders sometimes want to dismiss a card from the
  // active grid without it counting as completed (e.g. a delivery that's
  // running but they want to focus on the more urgent one above the fold).
  // Persisted to localStorage scoped to the rider's employeeId so logging
  // out / switching devices doesn't reshuffle their working set. The
  // "Show hidden" toggle replaces the active list with the hidden one so
  // they can unhide a row when they want it back.
  const hiddenKey = useMemo(
    () => (user?.employeeId ? `rider-hidden-deliveries:${user.employeeId}` : null),
    [user?.employeeId]
  );
  const [hiddenIds, setHiddenIds] = useState(() => {
    if (typeof window === 'undefined') return new Set();
    try {
      const raw = window.localStorage?.getItem(`rider-hidden-deliveries:${user?.employeeId || ''}`);
  ...
  const persistHidden = useCallback((next) => {
    ...
    try {
      ...
      // localStorage may be unavailable (Safari private mode); the in-memory
      // Set still works for the session — no need to surface this.
    }
  }, [hiddenKey]);
  const hideBooking = useCallback((id) => {
    setHiddenIds(prev => {
  ...
  const unhideBooking = useCallback((id) => {
    setHiddenIds(prev => {
      const next = new Set(prev);
      next.delete(id);
      persistHidden(next);
  ...
  const load = useCallback(async (isInitial = false) => {
    ...
    try {
      ...
      // Online bookings (Supabase-only table, no Dexie mirror) where this
      // rider was explicitly assigned via AdvanceBookingsTab. Without this
      // stream, an Owner could assign an online booking to a rider but the
      // rider's My Deliveries page would never see it — the assignment
      // would silently dead-end at the manager's screen.
      //
      // Belt-and-braces against the documented supabase.auth.getSession
      // hang (see project_supabase_hang.md): race the session lookup
      // against a 2s timeout so a stalled auth call can't block the
      // whole Promise.all and freeze the page on "Loading…". Worst case
      // we fall back to the anon key, which still satisfies the RLS
      // policy (SELECT requires only that the request comes from an
      // authenticated user; the JWT cookie set on sign-in suffices).
      const safeFetchOnlineBookings = async () => {
        try {
      ...
      const [advanceBookings, homeServices, transportRequests, onlineBookings] = await Promise.all([
        mockApi.advanceBooking.listAdvanceBookings(),
        safeListHomeServices(),
        safeListTransportRequests(),
        safeFetchOnlineBookings(),
      ]);
      ...
      // Normalize online_bookings to the same shape the rest of the page
      // expects. Field names mirror AdvanceBookingsTab.fetchOnlineBookings
      // so the booking card / modal render uniformly. Already filtered
      // server-side to rider_id = me, so no extra rider check here.
      const myOnline = (onlineBookings || []).map(ob => {
        const hour = parseInt(String(ob.preferred_time).split(':')[0] || '0', 10);
        const min = (String(ob.preferred_time).split(':')[1] || '0').padStart(2, '0');
        const bookingDateTime = `${ob.preferred_date}T${String(hour).padStart(2, '0')}:${min}:00`;
        const serviceNames = Array.isArray(ob.services)
          ? ob.services.map(s => s.name || s).join(' + ')
          : null;
        return {
          id: ob.id,
          ...
          paymentTiming: ob.payment_timing || (ob.payment_status === 'fully_paid' ? 'pay-now' : 'pay-after'),
      ...
      const merged = [...myAdvance, ...myOnline, ...homeServiceRows, ...transportRows];
      ...
      // The hidden-by-rider list is applied separately in the render layer so
      // toggling "Show hidden" doesn't require a refetch.
  ...
  useEffect(() => {
    ...
    const unsub = dataChangeEmitter.subscribe(c => {
      ...
      // Skip the echo of OUR OWN location writes — every 5s otherwise
      // reloads the entire deliveries list, causing the page to refresh
      // continuously while the rider has an active delivery. Window was
      // 3s but Supabase realtime can lag into the 3-5s tail before the
      // next GPS write refreshes the ref; widened to 10s for the same
      // reason as the Rooms page guard.
  ...
  useEffect(() => {
    ...
    const unsub = supabaseSyncManager.subscribe((status) => {
      ...
      // list included every `sync_complete` — but pushOnly() (called
      // via _debouncedPush on every local change) also fires
      // sync_complete with pulled=0. So every rider GPS write (5–10s)
      // triggered a push → sync_complete{pulled: 0} → reload. The page
      // appeared to "refresh of refresh." Gate sync_complete on
      // pulled > 0 so push-only confirmations no longer trigger a
      // local reload; realtime_update handles cross-device echoes for
      // the watched tables anyway.
      ...
      if (isRealtimeMatch || isPullWithChanges) {
        // Same self-write guard as the dataChangeEmitter subscriber above,
        // but on the realtime echo path. Widened to 10s for the same
    ...
    <div className="rider-bookings-page">
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: 16, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          {/* "Include untagged" toggle hidden from riders — admin-level
              UX. The legacy-record claim flow is the admin's job; the
              rider just sees their own deliveries. State + filter logic
              kept around in case a manager view re-enables this later. */}
          {/* "Show hidden" only renders when the rider has actually hidden
              something, to avoid header clutter in the common case. Clicking
              swaps the grid to show ONLY hidden cards so they can unhide a
              row; clicking again returns to the normal active view. */}
          {hiddenIds.size > 0 && (
            <button
              type="button"
              onClick={() => setShowHidden(v => !v)}
              style={{
                background: showHidden ? 'var(--primary, #6b1f2a)' : 'transparent',
                color: showHidden ? '#fff' : 'var(--primary, #6b1f2a)',
                border: '1.5px solid var(--primary, #6b1f2a)',
                borderRadius: 6,
                padding: '6px 12px',
                fontSize: '0.85rem',
                fontWeight: 500,
            ...
            >
              {showHidden ? '← Back to active' : `Show hidden (${hiddenIds.size})`}
            </button>
          )}
        </div>
      </div>
      ...
      {(() => {
        // Apply the rider's hide list at render time so toggling "Show
        // hidden" doesn't require a refetch. In normal view we drop any
        // booking the rider has hidden; in hidden view we keep ONLY
        // those, so they can unhide a row that they want back.
        const visibleBookings = showHidden
          ? bookings.filter(b => hiddenIds.has(b.id))
          : bookings.filter(b => !hiddenIds.has(b.id));
        return visibleBookings.length === 0 ? (
        <div className="empty-state">
          <h3>{showHidden ? 'No hidden deliveries' : 'No active deliveries'}</h3>
          <p>You're all caught up. New assignments will alert you here.</p>
          {/* The branch-mismatch / untagged-records diagnostic that used
              to render here was admin-level information (raw branch
              UUIDs, "Claim all" backfill buttons) and confused riders
              into thinking the empty state was an error. Branch tag
              hygiene is the admin's job to fix from the POS / employee
              pages; the rider page now just shows the clean "No active
              deliveries" message. The diag state + claim helpers are
              kept around so an admin can re-enable this panel later if
              we want to add a role-gated version. */}
        </div>
      ) : (
          ...
          {visibleBookings.map(b => {
            ...
            return (
              ...
              >
                ...
                {/* Hide / Unhide. stopPropagation so tapping the button
                    doesn't bubble up to the card's openCard handler and
                    pop the detail modal at the same time. */}
                <div style={{ marginTop: 10, display: 'flex', justifyContent: 'flex-end' }}>
                  {showHidden ? (
                    <button
                      type="button"
                      onClick={(e) => { e.stopPropagation(); unhideBooking(b.id); }}
                      style={{
                        background: 'transparent',
                        border: '1.5px solid var(--primary, #6b1f2a)',
                        color: 'var(--primary, #6b1f2a)',
                        borderRadius: 6,
                        padding: '4px 12px',
                        fontSize: '0.8rem',
                        cursor: 'pointer',
                    ...
                    <button
                      type="button"
                      onClick={(e) => { e.stopPropagation(); hideBooking(b.id); }}
                      style={{
                        background: 'transparent',
