...
export const transactionsAdapter = {
  ...
  // Mark a transaction as 'under_time': the therapist stopped the service
  // before the scheduled duration completed. Distinct from 'cancelled' —
  // payment, stock, commission, and rotation count all STAY because the
  // service did happen (just shorter than scheduled). We record the
  // actual vs scheduled duration so management can audit incidents and
  // Service History can show a dedicated UNDER TIME badge instead of
  // CANCELLED.
  async markTransactionUnderTime(id, { actualDuration, scheduledDuration, stoppedBy, stoppedByRole, reason } = {}) {
    await delay();
    return await db.transaction('rw', [db.transactions, db.syncQueue], async () => {
      const existing = await storageService.transactions.getById(id);
      if (!existing) throw new Error('Transaction not found');
      if (existing.status === 'cancelled') throw new Error('Transaction already cancelled');
      if (existing.status === 'voided') throw new Error('Transaction already voided');
...
export const attendanceAdapter = {
  ...
  async clockIn(employeeId, captureData = {}) {
    ...
    const record = await storageService.attendance.create({
      ...
      // Audit: which account actually pressed Clock In. Managers and
      // receptionists can clock employees in from Attendance.jsx, and
      // the employee can self-clock from MyPortal. Stamp the actor so
      // the photos modal can show "by Randy Benitua (Owner)" — without
      // this the attendance trail never distinguishes "the employee
      // showed up" from "a manager clocked them in remotely".
      clockedInBy: captureData.actorName || null,
      clockedInById: captureData.actorId || null,
      clockedInByRole: captureData.actorRole || null,
  ...
  async clockOut(employeeId, captureData = {}) {
    ...
    const updated = await storageService.attendance.update(record._id, {
      ...
      // Mirror the clockIn actor audit on the clockOut side. The two
      // halves of a shift can be performed by different accounts (e.g.
      // employee self-clocks in but forgets to clock out, manager
      // clocks them out at end-of-day), and only stamping both lets
      // the audit trail catch that.
      clockedOutBy: captureData.actorName || null,
      clockedOutById: captureData.actorId || null,
      clockedOutByRole: captureData.actorRole || null,
      isOutOfRange: captureData.isOutOfRange || false,
      // Heal orphan rows from before strict branch stamping was enforced —
