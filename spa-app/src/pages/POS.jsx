...
const POS = () => {
  ...
  const finalizeQrphCheckout = async () => {
    ...
    setReceiptData({
      ...
      tax: transaction.tax,
      isHomeService: transaction.isHomeService,
      homeServiceFee: transaction.homeServiceFee || 0,
                        ...
                        >
                          {customers
                            ...
                            .map(customer => (
                              <div
                                key={customer._id}
                                // Use onMouseDown + preventDefault instead of
                                // onClick. The input's onBlur schedules the
                                // dropdown to hide on a 200ms timer to allow
                                // a click to land — but onBlur fires before
                                // onClick, so on slower devices / certain
                                // browsers (Safari, in-app webviews) the
                                // dropdown is already gone by the time the
                                // click event arrives and the selection
                                // never registers. mousedown fires BEFORE
                                // blur, and preventDefault stops the focus
                                // shift entirely, so the suggestion stays
                                // mounted until our handler runs.
                                onMouseDown={(e) => {
                                  e.preventDefault();
                                ...
                                // Touch devices fire touchstart before mousedown
                                // (and some skip mousedown entirely if the touch
                                // moves or is preventDefaulted elsewhere). Mirror
                                // the same handler so a tap registers the pick
                                // immediately.
                                onTouchStart={(e) => {
                                  e.preventDefault();
                                  setSelectedCustomer(customer);
          ...
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '400px' }}>
            ...
            <div className="modal-body" id="receipt-content">
              ...
              <div style={{ fontSize: '0.9rem' }}>
                ...
                {receiptData.homeServiceFee > 0 && (
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span>Home Service Fee</span>
                    <span>₱{Number(receiptData.homeServiceFee).toLocaleString()}</span>
                  </div>
                )}
