...
const AdvanceBookingsTab = () => {
  const { user, showToast, isTherapist, canViewAll, hasManagementAccess, getEffectiveBranchId } = useApp();
  ...
  useEffect(() => {
    (async () => {
      ...
      // Scope the rider list to the manager's current branch. Without this
      // a Branch A manager would see Branch B's riders in the dropdown and
      // assign a booking to a rider who can't see it (the rider's own
      // My Deliveries page filters strictly by branchId). "All Branches"
      // mode (Owner) keeps the full list.
      const effectiveBranchId = getEffectiveBranchId?.();
      const inBranch = (e) => !effectiveBranchId || e.branchId === effectiveBranchId;
      setRiders(employees.filter(e => e.role === 'Rider' && e.status === 'active' && inBranch(e)));
    })();
  ...
  const handleAssignRider = async (booking, riderId) => {
    ...
    try {
      if (booking?.source === 'online') {
        // Online bookings live on a different Supabase table — the
        // advanceBooking adapter would silently miss the row. Route the
        // rider patch to online_bookings directly, mirroring the same
        // pattern handleConfirm / handleCancel / handleComplete already
        // use for online rows.
        await updateOnlineBookingRider(booking.id, {
          rider_id: riderId || null,
          rider_name: rider ? `${rider.firstName} ${rider.lastName}` : null,
          rider_assigned_at: riderId ? new Date().toISOString() : null,
          rider_assigned_by: user?.id || null,
        });
      } else {
  ...
  // PATCH an online booking with arbitrary rider fields. Separate from
  // updateOnlineBookingStatus because that one is hard-coded to write
  // status + confirmed_by + confirmed_at; this one writes whatever the
  // caller hands over so we can extend later (e.g. cancellation actor
  // fields) without forking yet another helper.
  const updateOnlineBookingRider = async (bookingId, patch) => {
    const supabaseUrl = import.meta.env.VITE_SUPABASE_URL;
    const supabaseKey = import.meta.env.VITE_SUPABASE_ANON_KEY;
