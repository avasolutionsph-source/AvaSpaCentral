...
const Rooms = ({ embedded = false, onDataChange, onOpenCreateRef, onManageOrderRef }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribeSync = supabaseSyncManager.subscribe((status) => {
      ...
      // Only reload on real server-sourced events. The previous trigger
      // list included every `sync_complete` — but pushOnly() (called via
      // _debouncedPush every time a local change is emitted) also fires
      // sync_complete with pulled=0. So a single GPS tick wrote home
      // service location → dataChange emit → debounced push → push
      // succeeds → sync_complete{pulled: 0} → Rooms reloaded → cards
      // repainted → page scrolled. Every 5 seconds. Gate sync_complete
      // on pulled > 0 so push-only confirmations no longer trigger a
      // local reload — realtime_update handles cross-device echoes for
      // the hot tables anyway.
      const isRealtimeMatch =
        status.type === 'realtime_update' && watched.includes(status.entityType);
      const isPullWithChanges =
        status.type === 'sync_complete' && Number(status.pulled || 0) > 0;

      if (isRealtimeMatch || isPullWithChanges) {
        // Self-write echo guard for realtime_update only. The therapist
        // writes her own GPS every 5s to each active home service. The
        // Supabase echo can land anywhere from ~200ms to ~5s later, and
        // a 3s window left a hole in the 3-5s tail before the next
        // write refreshed the ref. Bumped to 10s so the guard covers
        // the whole 5s interval plus laggy-network slack.
        if (
  ...
  // In-flight guard for the auto-expire poll. Without this, if a finalize
  // call fails (network error, transient Supabase write failure, etc.)
  // the next 5s tick sees the same expired card still in the list and
  // re-fires the same finalize — every 5 seconds, indefinitely. Each
  // attempt writes state and triggers a reload, which staff perceive as
  // continuous flicker on the Rooms page once a home service crosses
  // its scheduled end time. The Set holds IDs currently being finalized;
  // we add on entry and remove on exit (success OR failure) so a single
  // bad attempt doesn't permanently lock the card out of completion.
  const finalizingRoomsRef = useRef(new Set());
  const finalizingHomeServicesRef = useRef(new Set());

  // Normal-completion finalizer for a room. Used both by the auto-expire
  // poll AND the manual "Mark Complete" button (visible once the timer
  // hits 00:00). This is the "Time's Up" path — full charge, full
  // commission, no incident notifications, room released. Distinct from
  // handleStopService which is for *interrupting* an in-progress service.
  const finalizeCompletedRoom = async (room, { silent = false } = {}) => {
    if (finalizingRoomsRef.current.has(room._id)) return;
    finalizingRoomsRef.current.add(room._id);
    try {
      // Pay-after advance booking: create the transaction now and close the booking.
      if (room.advanceBookingId && room.paymentTiming === 'pay-after') {
        try {
          ...
          if (!silent) showToast(`${room.name} service completed and payment recorded`, 'success');
        } catch (bookingError) {
          console.error(`Failed to process pay-after booking for ${room.name}:`, bookingError);
          if (!silent) showToast(`${room.name} service completed but payment recording failed. Please process manually.`, 'warning');
        }
      }
      await mockApi.rooms.updateRoomStatus(room._id, 'available');
      if (!silent) showToast(`${room.name} service completed - room now available`, 'info');
      loadRooms();
    } catch (error) {
      console.error('finalizeCompletedRoom failed:', error);
    } finally {
      finalizingRoomsRef.current.delete(room._id);
    }
  };

  // Normal-completion finalizer for a home service. Mirrors
  // finalizeCompletedRoom: used both by the auto-expire poll and the
  // manual "Mark Complete" button.
  const finalizeCompletedHomeService = async (service, { silent = false } = {}) => {
    if (finalizingHomeServicesRef.current.has(service._id)) return;
    finalizingHomeServicesRef.current.add(service._id);
    try {
      if (service.isAdvanceBooking) {
        try {
          ...
          if (booking.paymentTiming === 'pay-after') {
            await createPayAfterTransaction(booking);
            if (!silent) showToast(`Home service completed and payment recorded`, 'success');
          }
          await mockApi.advanceBooking.completeServiceFromBooking(service._id, {
            paymentMethod: 'Cash'
          });
        } catch (bookingError) {
          console.error('finalizeCompletedHomeService booking error:', bookingError);
        }
      } else {
        // Mark regular home service as completed (NOT delete) so the
        // rider page still has an audit trail; rider's default filter
        // hides 'completed'.
        await homeServicesApi.updateHomeServiceStatus(service._id, 'completed', {
          completedBy: user?.name || 'staff',
        });
      }
      if (!silent) showToast(`Home service for ${service.customerName} completed`, 'info');
      loadHomeServices();
    } catch (error) {
      console.error('finalizeCompletedHomeService failed:', error);
    } finally {
      finalizingHomeServicesRef.current.delete(service._id);
    }
  };

  // Manual "Mark Complete (Time's Up)" handler for a room — same flow as
  // the auto-expire poll but explicit user action. Shown when the timer
  // hits 00:00 in case the auto-poll hasn't fired yet (e.g. Rooms page
  // was closed when the timer expired).
  const handleMarkRoomComplete = async (room) => {
    await finalizeCompletedRoom(room);
  };

  const handleMarkHomeServiceComplete = async (service) => {
    await finalizeCompletedHomeService(service);
  };

  // Auto-complete rooms when timer expires. Important: do NOT include
  // `currentTime` (the once-per-second display tick) in the deps. With it
  // in the deps the effect tears down and re-creates the 5s interval on
  // every render, which means the interval never actually matures —
  // checkExpiredRooms only ever runs from the `rooms` change re-bind,
  // not from the timer. Compute expiry from Date.now() inside the closure
  // so the check uses real-time without React state coupling. This also
  // removes a per-second source of re-bind churn on the Rooms page.
  useEffect(() => {
    const checkExpiredRooms = async () => {
      const now = Date.now();
      for (const room of rooms) {
        if (room.status === 'occupied' && room.startTime && room.serviceDuration) {
          const expiryMs = new Date(room.startTime).getTime() + room.serviceDuration * 60000;
          if (now >= expiryMs) {
            await finalizeCompletedRoom(room);
          }
        }
  ...
  // Auto-complete home services when timer expires. Same deps-fix as above:
  // currentTime stays out so the 5s interval is allowed to mature.
  useEffect(() => {
    const checkExpiredHomeServices = async () => {
      const now = Date.now();
      for (const service of homeServices) {
        if (service.status === 'occupied' && service.startTime && service.serviceDuration) {
          const expiryMs = new Date(service.startTime).getTime() + service.serviceDuration * 60000;
          if (now >= expiryMs) {
            await finalizeCompletedHomeService(service);
          }
        }
  ...
  // Resolve a transaction row by receiptNumber and flip its status to
  // 'under_time' with actual vs scheduled duration stamped. Used when a
  // therapist stops an in-progress service before the scheduled duration
  // completes. Distinct from cancelLinkedTransaction: the transaction stays
  // billable (commission + revenue + stock all preserved). Silent no-op when
  // there's no linked transaction yet (pay-after path before createTransaction).
  const markLinkedTransactionUnderTime = async (receiptNumber, payload) => {
    if (!receiptNumber) return;
    try {
      const txn = await mockApi.transactions.getTransactionByReceiptNumber(receiptNumber);
      if (!txn || !txn._id) return;
      if (txn.status === 'cancelled' || txn.status === 'voided') return;
      await mockApi.transactions.markTransactionUnderTime(txn._id, payload);
    } catch (err) {
      console.warn('[Rooms] markLinkedTransactionUnderTime failed:', err);
    }
  };

  // Fan out the under-time incident to (1) the therapist who was assigned
  // the room — a direct disciplinary nudge — and (2) the manager-tier roles
  // for the branch as an incident report. Uses the same role spelling
  // ('Manager', 'Owner', 'Branch Owner') as inventory / daily triggers so
  // the broadcast lands on the same audience as other escalations.
  const notifyUnderTimeIncident = async ({
    employeeId,
    employeeName,
    customerName,
    branchId,
    ...
    const remaining = Math.max(0, (scheduledDuration || 0) - (actualDuration || 0));
    try {
      if (employeeId) {
        await NotificationService.notify({
          type: NotificationService.TYPES.SERVICE_UNDER_TIME,
          targetUserId: employeeId,
          branchId: branchId || null,
          title: 'Under Time Service',
          message: `You did not complete the required duration of the massage session and were marked under time (${actualDuration}/${scheduledDuration} min, ${remaining} min remaining). This incident will be reported to management.`,
          soundClass: 'oneshot',
          payload: { transactionId, actualDuration, scheduledDuration, reason },
        });
      }
      await NotificationService.notify({
        type: NotificationService.TYPES.SERVICE_UNDER_TIME,
        ...
        branchId: branchId || null,
        title: 'Under Time Incident',
        message: `${employeeName || 'A therapist'} stopped service for ${customerName || 'a customer'} at ${actualDuration}/${scheduledDuration} min. Reason: ${reason}. Stopped by: ${stoppedByName}.`,
        action: '/service-history',
        actionLabel: 'Review',
        soundClass: 'oneshot',
        payload: { transactionId, employeeId, actualDuration, scheduledDuration, reason },
      });
    } catch (err) {
      console.warn('[Rooms] notifyUnderTimeIncident failed:', err);
    }
  };
  ...
  const handleStopService = async () => {
    ...
    // Resolve the actor (who's stopping) once up front so we can stamp it
    ...
    try {
      ...
      //   - Service never started (room.status === 'pending') → CANCELLED.
      //     No service happened — void the transaction, restore stock,
      //     undo the rotation. No commission, no revenue.
      //   - Service started AND timer hit zero → COMPLETED. This is the
      //     "Time's Up" path — the user came through the Stop modal when
      //     they probably wanted Mark Complete. Treat it as a normal
      //     finish: full pay, full commission, no incident notification.
      //     We bail out of the Stop flow entirely and call the finalize
      //     helper, which is the same code path as the auto-expire poll.
      //   - Service started AND timer still has time left → UNDER TIME.
      //     Therapist stopped early — keep the transaction billable
      //     (full charge + commission) but flag it with actualDuration vs
      //     scheduledDuration and notify the therapist + managers as an
      //     incident.
      const now = new Date();
      let actualDuration = 0;
      let scheduledDuration = room.serviceDuration || 0;
      let status = 'cancelled';

      if (room.status === 'occupied' && room.startTime) {
        const startTime = new Date(room.startTime);
        actualDuration = Math.floor((now - startTime) / 60000); // minutes
        const remaining = isHomeService ? getHomeServiceRemainingTime(room) : getRemainingTime(room);
        if (remaining && remaining.isExpired) {
          // Times up — route to the normal completion path and exit.
          if (isHomeService) {
            await finalizeCompletedHomeService(room);
          } else {
            await finalizeCompletedRoom(room);
          }
      ...
      if (isHomeService) {
        ...
        if (room.isAdvanceBooking) {
          // If service was in progress (under time), create transaction for
          // pay-after and complete — then mark the new transaction under_time.
          if (status === 'under_time' && room.paymentTiming === 'pay-after') {
            try {
              const booking = await mockApi.advanceBooking.getAdvanceBooking(room._id);
              const created = await createPayAfterTransaction(booking);
              await mockApi.advanceBooking.completeServiceFromBooking(room._id, {
                paymentMethod: 'Cash'
              });
              if (created?.receiptNumber) {
                await markLinkedTransactionUnderTime(created.receiptNumber, underTimePayload);
              }
              showToast(`Home service marked under time and payment recorded`, 'success');
            } catch (bookingError) {
              // Silent fail for pay-after completion
            }
          } else if (status === 'under_time') {
            // pay-now advance booking with a service that started: transaction
            // already exists. Flip it to under_time; do NOT cancel the booking.
            await mockApi.advanceBooking.completeServiceFromBooking(room._id, {
              paymentMethod: 'Cash'
            }).catch(() => {});
            await markLinkedTransactionUnderTime(room.transactionId, underTimePayload);
            showToast(`Home service marked under time`, 'success');
          } else {
            // status === 'cancelled' — service never started
            ...
            showToast(`Home service cancelled: ${stopReason}`, 'info');
          }
        } else {
          // Regular (non-advance) home service. If it started, mark home
          // service 'completed' (so it falls off the rider's active list)
          // and flip the transaction to under_time. If it never started,
          // cancel the home service + linked transaction.
          if (status === 'under_time') {
            await homeServicesApi.updateHomeServiceStatus(room._id, 'completed', {
              completedBy: actorName,
              notes: `Under time — ${actualDuration}/${scheduledDuration} min. Reason: ${stopReason}`,
            });
            await markLinkedTransactionUnderTime(room.transactionId, underTimePayload);
            showToast(`Home service marked under time`, 'success');
          } else {
        ...
        if (status === 'under_time') {
          await notifyUnderTimeIncident({
            employeeId: room.employeeId || room.assignedEmployeeId,
            employeeName: room.employeeName || room.assignedEmployeeName,
            ...
            scheduledDuration,
            reason: stopReason,
            stoppedByName: actorDisplay,
          });
        }

        loadHomeServices();
      } else {
        // Room-based service path.
        if (status === 'under_time') {
          // Service started — under-time path. Pay-after: create the txn,
          // complete the booking, then flip status. Pay-now / walk-in: txn
          // already exists, just flip its status.
          if (room.advanceBookingId && room.paymentTiming === 'pay-after') {
            try {
              const booking = await mockApi.advanceBooking.getAdvanceBooking(room.advanceBookingId);
              const created = await createPayAfterTransaction(booking);
              ...
              showToast(`Service marked under time and payment recorded`, 'success');
            } catch (bookingError) {
              // Silent fail for pay-after completion
            }
          } else {
            if (room.advanceBookingId) {
              // pay-now advance booking — complete the booking record but
              // keep the existing transaction (now under_time).
              await mockApi.advanceBooking.completeServiceFromBooking(room.advanceBookingId, {
                paymentMethod: 'Cash'
              }).catch(() => {});
            }
            await markLinkedTransactionUnderTime(room.transactionId, underTimePayload);
            showToast(`Service marked under time`, 'success');
          }
        } else {
          // status === 'cancelled' — service never started.
          if (room.advanceBookingId) {
          ...
          await cancelLinkedTransaction(room.transactionId, stopReason, actorDisplay, actorRole);
          // Undo the service rotation count — therapist didn't actually serve.
          ...
          showToast(`Service cancelled: ${stopReason}`, 'info');
        }

        // Either way the room is released to 'available'.
        await mockApi.rooms.updateRoomStatus(room._id, 'available');

        if (status === 'under_time') {
          await notifyUnderTimeIncident({
            employeeId: room.assignedEmployeeId,
            employeeName: room.assignedEmployeeName,
            customerName: room.customerName,
            branchId: room.branchId,
            transactionId: room.transactionId,
            actualDuration,
  ...
  return (
    <div className="rooms-page">
            ...
            return (
              <div key={room._id} className={`room-card ${room.status}`}>
                ...
                {room.status === 'occupied' && (isMyRoom || !isTherapist()) && (() => {
                  // Show "Mark Complete" (green) once the timer reaches
                  // 00:00 so anyone can immediately free the room and the
                  // therapist becomes assignable again, even if the
                  // auto-expire poll hasn't fired yet (e.g. Rooms tab was
                  // closed when the timer hit zero). Before time's up,
                  // keep the existing Stop button which routes to the
                  // under_time path with reason + notifications.
                  const timer = getRemainingTime(room);
                  const isExpired = timer && timer.isExpired;
                  return (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginTop: 'var(--spacing-sm)' }}>
                      ...
                      {isExpired ? (
                        <button
                          ...
                          onClick={() => handleMarkRoomComplete(room)}
                          style={{ width: '100%', fontSize: '0.85rem', padding: '6px 10px' }}
                        >
                          ✅ Mark Complete (Time's Up)
                        </button>
                      ) : (
                        <button
      ...
      {/* Pahatid — prominent action button always visible on Rooms. The
          header has the same affordance as a compact icon for cross-page
          access, but therapists / receptionists working in Rooms get a
              ...
              return (
                <div key={service._id} className={`home-service-card ${service.status} ${service.isAdvanceBooking ? 'advance-booking' : ''}`}>
                  ...
                  {/* Action buttons for occupied home services — mirror
                      ...
                      early without leaving the page. Once the timer hits
                      00:00, swap Stop for a green "Mark Complete" button
                      so the home service can be released for the next
                      assignment even if the auto-expire poll hasn't run. */}
                  {service.status === 'occupied' && (isMyService || !isTherapist()) && (() => {
                    const timer = getHomeServiceRemainingTime(service);
                    const isExpired = timer && timer.isExpired;
                    return (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginTop: 'var(--spacing-sm)' }}>
                        <button
                        ...
                        {isExpired ? (
                          <button
                            className="btn btn-success"
                            onClick={() => handleMarkHomeServiceComplete(service)}
                            style={{ width: '100%', fontSize: '0.85rem', padding: '6px 10px' }}
                          >
                            ✅ Mark Complete (Time's Up)
                          </button>
            ...
            <div className="modal-body">
              <div className="stop-service-info">
                ...
                {stopServiceModal.room?.status === 'occupied' && stopServiceModal.room?.startTime && (() => {
                  // Surface the elapsed vs scheduled split so the operator
                  // sees up front whether this stop will be flagged as
                  // UNDER TIME (timer still has time left) or COMPLETED
                  // (timer already at 00:00). Mirrors the same numbers
                  // we'll persist on the transaction + the disciplinary
                  // notification.
                  const scheduled = stopServiceModal.room.serviceDuration || 0;
                  const elapsed = Math.floor((new Date() - new Date(stopServiceModal.room.startTime)) / 60000);
                  const remaining = Math.max(0, scheduled - elapsed);
                  const willBeUnderTime = remaining > 0;
                  return (
                    <>
                      <p><strong>Scheduled:</strong> {scheduled} min</p>
                      <p><strong>Running for:</strong> {elapsed} min</p>
                      <p><strong>Time remaining:</strong> {remaining} min</p>
                      {willBeUnderTime ? (
                        <div style={{ marginTop: '8px', padding: '8px 12px', background: '#fffbeb', border: '1px solid #fcd34d', borderRadius: '6px', fontSize: '0.85rem' }}>
                          <strong style={{ color: '#b45309' }}>⚠️ This will be marked UNDER TIME.</strong>{' '}
                          The therapist and managers will be notified. Customer is still charged in full and commission is preserved.
                        </div>
                      ) : (
                        <div style={{ marginTop: '8px', padding: '8px 12px', background: '#ecfdf5', border: '1px solid #6ee7b7', borderRadius: '6px', fontSize: '0.85rem' }}>
                          <strong style={{ color: '#047857' }}>✓ Timer already complete.</strong>{' '}
                          Stopping now will close the receipt as COMPLETED (normal finish).
                        </div>
                      )}
                    </>
                  );
