  ...
  const fetchTransactions = async () => {
    ...
    try {
      ...
      const formattedTransactions = apiTransactions.map((t, index) => {
        ...
        underTimeAt: t.underTimeAt,
        actualDuration: t.actualDuration,
        scheduledDuration: t.scheduledDuration,
        stoppedBy: t.stoppedBy,
        stoppedByRole: t.stoppedByRole,
                  ...
                  <tr key={transaction.id} onClick={() => handleViewDetail(transaction)}
                    ...
                    <td>
                      <span className="transaction-receipt">
                        ...
                        {transaction.status === 'under_time' && (
                          <span
                            title={`Under time — ${transaction.actualDuration ?? '?'}/${transaction.scheduledDuration ?? '?'} min${transaction.stoppedBy ? ` (stopped by ${transaction.stoppedBy})` : ''}${transaction.stopReason ? ` — ${transaction.stopReason}` : ''}`}
                            style={{ marginLeft: '6px', background: '#f59e0b', color: '#fff', fontSize: '0.65rem', padding: '2px 6px', borderRadius: '4px', textDecoration: 'none', display: 'inline-block' }}>
                            UNDER TIME
                          </span>
                        )}
        ...
        <div className="modal-overlay" onClick={() => setShowDetailModal(false)}>
          <div className="modal-content transaction-detail-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="transaction-info-grid">
              ...
              {selectedTransaction.status === 'under_time' && (
              <div className="info-group" style={{ background: '#fffbeb', padding: '8px 12px', borderRadius: '6px', border: '1px solid #fcd34d' }}>
                <div className="info-label" style={{ color: '#b45309', fontWeight: 700 }}>UNDER TIME</div>
                <div className="info-value" style={{ fontSize: '0.85rem' }}>
                  {(selectedTransaction.actualDuration != null && selectedTransaction.scheduledDuration != null) && (
                    <>Duration: {selectedTransaction.actualDuration}/{selectedTransaction.scheduledDuration} min<br/></>
                  )}
                  {selectedTransaction.stopReason}<br/>
                  <span style={{ color: '#999', fontSize: '0.75rem' }}>
                    by {selectedTransaction.stoppedBy || 'Unknown'}
                    {selectedTransaction.underTimeAt && ` on ${format(parseISO(selectedTransaction.underTimeAt), 'MMM dd, yyyy h:mm a')}`}
                  </span>
                </div>
              </div>
              )}
            ...
            <div className="payment-summary">
              ...
              {(() => {
                const sub = Number(selectedTransaction.subtotal) || 0;
                const disc = Number(selectedTransaction.discount) || 0;
                const tx = Number(selectedTransaction.tax) || 0;
                const tot = Number(selectedTransaction.total) || 0;
                const stored = Number(selectedTransaction.homeServiceFee) || 0;
                const derived = Math.max(0, Math.round((tot - (sub - disc + tx)) * 100) / 100);
                const fee = stored > 0 ? stored : derived;
                return fee > 0 ? (
                  <div className="summary-line">
                    <span>Home Service Fee:</span>
                    <span className="summary-value">₱{fee.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</span>
                  </div>
                ) : null;
              })()}
