...
export const analyticsApi = {
  ...
  async getSalesHeatmapData() {
    ...
    const allTransactions = await getData.transactions();
    // Exclude voided/cancelled — they don't represent actual sales. under_time
    // is kept (paid by customer, therapist just stopped early).
    const transactions = allTransactions.filter(t =>
      t.date && new Date(t.date) >= last30Days && t.status !== 'voided' && t.status !== 'cancelled'
    );
