-- supabase/setup-steps/45-add-pull-indexes-for-sync.sql
-- Run this in Supabase SQL Editor as step 45.
...
--
-- Without (business_id, updated_at DESC), Postgres falls back to either the
-- single-column business_id index plus an in-memory sort, or a full table
-- scan when no updated_at filter is supplied. pg_stat_statements flagged the
-- unfiltered attendance pull as ~20% of total DB time on the AVADAETSPA
-- project before this change.
--
-- These indexes are deliberately restricted to the tables that genuinely
-- have an updated_at column AND see meaningful row volume. Smaller tables
-- (settings, saved_payrolls, etc.) stay on the existing business_id index.
--
